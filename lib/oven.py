...
try:
    ...
    if config.max31855spi:
        ...
        if config.get_gpio_heat(1) in spi_reserved_gpio:
...
class Oven(threading.Thread):
    ...
    def __init__(self, channel=1, simulate=False, time_step=config.sensor_time_wait):
        ...
        if value:
            ...
            if gpio_available:
                if config.heater_invert:
                    GPIO.output(config.get_gpio_heat(self.channel), GPIO.LOW)
                else:
                    GPIO.output(config.get_gpio_heat(self.channel), GPIO.HIGH)
        else:
            self.heat = 0.0
            if gpio_available:
                if config.heater_invert:
    ...
    def __init__(self, time_step, oven):
        ...
        if config.max6675:
            ...
            self.thermocouple = MAX6675(config.gpio_sensor_cs,
                                        ...
                                        config.get_gpio_sensor_data(oven.channel),
                                        config.temp_scale)
